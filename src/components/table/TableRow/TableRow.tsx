...
const TableRow = ({ rowData, rowType }: RowProps) => {
  const { switchReducer, deleteNote, moveNote, moveAll, deleteAll, toggleModal } = useActions();
  ...
  return (
    ...
    >
      {rowData.map((item, index) => {
        ...
        const isImageCell =
          (index === 1 && rowType === 'note') || (index === 0 && rowType === 'stat');
        return (
          <td key={crypto.randomUUID()} className={rowType.includes('stat') ? 'w-96' : ''}>
            {isImageCell ? <img src={item} /> : <span className='truncate'>{item}</span>}
          </td>
        );
      })}
      {rowType === 'note' ? (
